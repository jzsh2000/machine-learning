...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if not testing:
            ...
            self.epsilon = math.fabs(math.cos(0.01 * self.trials))
        else:
            self.epsilon = 0.0
            self.alpha = 0.0
    ...
    def build_state(self):
        ...
        if self.learning:
            if not str(state) in self.Q.keys():
                self.Q[str(state)] = dict(zip(self.valid_actions, [0] * len(self.valid_actions)))
    ...
    def createQ(self, state):
        ...
        if self.learning:
            if not str(state) in self.Q.keys():
                self.Q[str(state)] = dict(zip(self.valid_actions, [0.0] * len(self.valid_actions)))
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if self.epsilon > 0.01 and self.epsilon > random.random():
                ...
                action = [ key for key in self.Q[str(state)] if self.Q[str(state)][key] == maxQ ][0]
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            self.Q[str(state)][action] = currentQ + self.alpha * (reward - currentQ)
